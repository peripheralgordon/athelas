<script>
    ...
    let questions = [
        { id: 1, text: `Quotidianamente`, value: 3 },
        { id: 2, text: `Di tanto in tanto`, value: 5 },
        { id: 3, text: `Molto raramente`, value: 8 },
        { id: 4, text: `Mai`, value: 10 },
    ];

    // user age score
    let smokeScore = 10; // default selection
...
<div class="flex flex-col justify-between overflow-hidden bg-white pb-6">
    <div in:fly="{{ x: -1000, duration: 200 }}" class="mb-3 bg-blue-700  rounded-lg text-white w-fit">
        <p class="p-2 pl-4 pr-4">Fumi?</p>
    </div>

    <select bind:value={smokeScore} class="form-select form-select-md mb-3
        appearance-none
        block
        w-full
        px-3
