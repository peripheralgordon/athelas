<script>
    ...
    let questions = [
        { id: 1, text: `Molto`, value: 10 },
        { id: 2, text: `Discreto`, value: 8 },
        { id: 3, text: `Poco`, value: 5 },
        { id: 3, text: `Molto poco`, value: 3 },
    ];

    // user age score
    let workScore = 10; // default selection
...
<div class="flex flex-col justify-between overflow-hidden bg-white pb-6">
    <div in:fly="{{ x: -1000, duration: 200 }}" class="mb-3 bg-blue-700  rounded-lg text-white w-fit">
        <p class="p-2 pl-4 pr-4">Quanto movimento compi durante l'orario lavorativo?</p>
    </div>

    <select bind:value={workScore} class="form-select form-select-md mb-3
        appearance-none
        block
        w-full
        px-3
